...
@Component
@Slf4j
public class ParkEntranceHisInHandlerV1 extends AbstractCanalHandler {

    private final String eventName = "parkEntranceHisIn";
    ...
    public boolean handle(Entry canalEntry) {
        ...
        for (RowData rowData : rowChange.getRowDatasList()) {
            Column picColumn = this.getByName(rowData.getAfterColumnsList(), "enterPicUrl");
            if (picColumn == null || !picColumn.getUpdated()) {
                //入场图片为未更新或无字段，跳过事件（入场、入场抓拍均会触发该事件）
                return next();
            }

            List<Column> columnList = this.getListByName(rowData.getAfterColumnsList(), "projectId", "plateNumber", "enterTime", "enterGateName", "enterPicUrl");
