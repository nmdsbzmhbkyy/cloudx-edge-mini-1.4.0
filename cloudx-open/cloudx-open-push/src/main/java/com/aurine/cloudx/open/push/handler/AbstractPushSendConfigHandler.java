...
import com.aurine.cloudx.open.push.factory.PushSendConfigMachineFactory;
...
public abstract class AbstractPushSendConfigHandler extends PushSendCommonHandler implements PushSendHandler {
    ...
    protected HandleResult<OpenPushModel> preHandle(OpenPushModel model, String version) {
        ...
        if (StringUtils.isNotEmpty(serviceName)) {
            try {
                ...
                PushSendConfigMachine machine = PushSendConfigMachineFactory.getInstance().getMachine(serviceName, version);
                if (machine != null) {
                    // 目前配置类的方法只有一个，所以不需要利用反射，直接调用即可
                    result.setData(machine.doConfig(model));
                }
    ...
    public String getServiceType() {
        return OpenPushSubscribeCallbackTypeEnum.CONFIG.name;
    }
