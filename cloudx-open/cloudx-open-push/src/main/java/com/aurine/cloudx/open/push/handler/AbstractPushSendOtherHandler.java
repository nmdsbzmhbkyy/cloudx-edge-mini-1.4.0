...
import com.aurine.cloudx.open.push.factory.PushSendOtherMachineFactory;
...
public abstract class AbstractPushSendOtherHandler extends PushSendCommonHandler implements PushSendHandler {
    ...
    protected HandleResult<OpenPushModel> preHandle(OpenPushModel model, String version) {
        ...
        if (StringUtils.isNotEmpty(serviceName)) {
            try {
                ...
                PushSendOtherMachine machine = PushSendOtherMachineFactory.getInstance().getMachine(serviceName, version);
                if (machine != null) {
                    // 目前其他类的方法只有一个，所以不需要利用反射，直接调用即可
                    result.setData(machine.other(model));
                }
    ...
    public String getServiceType() {
        return OpenPushSubscribeCallbackTypeEnum.OTHER.name;
    }
