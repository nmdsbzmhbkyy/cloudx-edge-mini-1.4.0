...
import com.aurine.cloudx.open.push.handler.AbstractPushSendOperateHandler;
...
@Slf4j
@Component
public class PushSendOperateHandler extends AbstractPushSendOperateHandler {

    @Override
    public R handle(OpenPushModel model) {
        HandleResult<OpenPushModel> handleResult = super.preHandle(model, getVersion());
        log.info("[PushSendOperateHandler - handle]: 处理结束的数据, handleResult={}", JSONConvertUtils.objectToString(handleResult));
        ...
        if (callbackResult == null) {
            log.info("[PushSendOperateHandler - handle]: 未找到回调地址");
            return Result.fail(model, CloudxOpenErrorEnum.NOT_FOUND_CALLBACK);
        }
        log.info("[PushSendOperateHandler - handle]: 调用回调的结果, callbackResult={}", callbackResult);
