...
import com.aurine.cloudx.open.push.handler.AbstractPushSendEventHandler;
...
@Slf4j
@Component
public class PushSendEventHandler extends AbstractPushSendEventHandler {

    @Override
    public R handle(OpenPushModel model) {
        HandleResult<OpenPushModel> handleResult = super.preHandle(model, getVersion());
        log.info("[PushSendEventHandler - handle]: 处理结束的数据, handleResult={}", JSONConvertUtils.objectToString(handleResult));
        ...
        if (callbackResult == null) {
            log.info("[PushSendEventHandler - handle]: 未找到回调地址");
            return Result.fail(model, CloudxOpenErrorEnum.NOT_FOUND_CALLBACK);
        }
        log.info("[PushSendEventHandler - handle]: 调用回调的结果, callbackResult={}", callbackResult);
