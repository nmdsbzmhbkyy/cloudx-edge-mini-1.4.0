...
import com.aurine.cloudx.open.origin.constant.enums.ApproveStatusEnum;
import com.aurine.cloudx.open.origin.entity.*;
import com.aurine.cloudx.open.origin.mapper.ProjectComplaintRecordMapper;
...
public class ProjectComplaintRecordServiceImpl extends ServiceImpl<ProjectComplaintRecordMapper, ProjectComplaintRecord> implements ProjectComplaintRecordService {
    ...
    public R setHandler(String complaintId, String handler) {
        ...
        ProjectComplaintRecordInfoVo  complainVo= getByComplainId(complaintId);
        if (ObjectUtil.isNotEmpty(projectComplaintRecord)) {
            ...
            String statusType=projectRepairRecordService.getStatusByType(projectComplaintRecord.getStatus(),"complaint_status");
            try {

                noticeUtil.send(true, "投诉建议通知",
                                "您有被指派的投诉工单，请尽快处理<br />" +
                                "联系人："+complainVo.getPersonName()+"<br />" +
                                "电话："+complainVo.getPhoneNumber()+"<br />" +
                                "房号："+complainVo.getCurrentHouseName()+"<br />" +
                                "投诉建议内容："+projectComplaintRecord.getContent()+"<br />" +
                                "时间："+ DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss").format(projectComplaintRecord.getCreateTime())+"<br />",
                        handler);
                ...
                noticeUtil.send(false, "投诉处理进展通知",
                        "投诉状态变更\n" +
                                "投诉人房号："+complainVo.getCurrentHouseName() +"<br />" +
                                "投诉主题：  "+projectComplaintRecord.getContent()+"  <br />" +
                                "投诉时间："+ DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss").format(projectComplaintRecord.getCreateTime())+"<br />" +
                                "当前进展："+statusType+"<br />",
    ...
    public R setResult(ProjectComplainRecordResultFormVo formVo) {
        ...
        ProjectComplaintRecordInfoVo  complainVo= getByComplainId(formVo.getComplaintId());
