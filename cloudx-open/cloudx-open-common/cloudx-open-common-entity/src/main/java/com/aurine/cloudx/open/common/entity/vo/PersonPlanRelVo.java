...
@ApiModel(value = "人员通行方案关系Vo")
public class PersonPlanRelVo extends OpenBaseVo {
    ...
    @NotBlank(message = "通行方案id（planId）不能为空", groups = {InsertGroup.class})
    @SizeCustom(message = "通行方案id（planId）长度不能小于1")
    @Size(max = 32, message = "通行方案id（planId）长度不能超过32")
    private String planId;

    /**
     * 是否启用，与该方案关联的人员是否处于启用状态
     */
