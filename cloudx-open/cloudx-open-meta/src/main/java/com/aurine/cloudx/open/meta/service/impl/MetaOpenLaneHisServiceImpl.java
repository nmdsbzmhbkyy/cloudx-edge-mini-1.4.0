package com.aurine.cloudx.open.meta.service.impl;

import com.aurine.cloudx.open.common.core.exception.CloudxOpenErrorEnum;
import com.aurine.cloudx.open.common.core.response.Result;
import com.aurine.cloudx.open.meta.service.MetaOpenLaneHisService;
import com.aurine.cloudx.open.origin.entity.ProjectOpenLaneHis;
import com.aurine.cloudx.open.origin.service.ProjectOpenLaneHisService;
import com.pig4cloud.pigx.common.core.util.R;
import org.springframework.stereotype.Service;

import javax.annotation.Resource;

/**
 * open平台-开关闸记录管理实现
 *
 * @author : Qiu
 * @date : 2021 12 28 9:55
 */

@Service
public class MetaOpenLaneHisServiceImpl implements MetaOpenLaneHisService {

    @Resource
    private ProjectOpenLaneHisService projectOpenLaneHisService;


    @Override
    public R<ProjectOpenLaneHis> save(ProjectOpenLaneHis po) {
        boolean result = projectOpenLaneHisService.saveOrUpdate(po);
        if (!result) return Result.fail(po, CloudxOpenErrorEnum.SYSTEM_ERROR);

        return R.ok(po);
    }

    @Override
    public R<ProjectOpenLaneHis> update(ProjectOpenLaneHis po) {
        boolean result = projectOpenLaneHisService.saveOrUpdate(po);
        if (!result) return Result.fail(po, CloudxOpenErrorEnum.EMPTY_RESULT);

        return R.ok(po);
    }

    @Override
    public R<Boolean> delete(String id) {
        boolean result = projectOpenLaneHisService.removeById(id);
        if (!result) return Result.fail(false, CloudxOpenErrorEnum.EMPTY_RESULT);

        return R.ok(true);
    }
}
