package com.aurine.cloudx.open.meta.service.impl;

import com.aurine.cloudx.open.common.core.response.Result;
import com.aurine.cloudx.open.origin.entity.ProjectHousePersonRel;
import com.aurine.cloudx.open.common.core.exception.CloudxOpenErrorEnum;
import com.aurine.cloudx.open.meta.service.MetaHousePersonInfoService;
import com.aurine.cloudx.open.origin.service.ProjectHousePersonRelService;
import com.pig4cloud.pigx.common.core.util.R;
import org.springframework.stereotype.Service;

import javax.annotation.Resource;

/**
 * open平台-住户信息管理实现
 *
 * @author : Qiu
 * @date : 2021 12 28 9:55
 */

@Service
public class MetaHousePersonInfoServiceImpl implements MetaHousePersonInfoService {

    @Resource
    private ProjectHousePersonRelService projectHousePersonRelService;
    

    @Override
        public R<ProjectHousePersonRel> save(ProjectHousePersonRel po) {
        boolean result = projectHousePersonRelService.saveOrUpdate(po);
        if (!result) return Result.fail(po, CloudxOpenErrorEnum.SYSTEM_ERROR);

        return R.ok(po);
    }

    @Override
    public R<ProjectHousePersonRel> update(ProjectHousePersonRel po) {
        boolean result = projectHousePersonRelService.saveOrUpdate(po);
        if (!result) return Result.fail(po, CloudxOpenErrorEnum.EMPTY_RESULT);

        return R.ok(po);
    }

    @Override
    public R<Boolean> delete(String id) {
        boolean result = projectHousePersonRelService.removeById(id);
        if (!result) return Result.fail(false, CloudxOpenErrorEnum.EMPTY_RESULT);

        return R.ok(true);
    }
}
