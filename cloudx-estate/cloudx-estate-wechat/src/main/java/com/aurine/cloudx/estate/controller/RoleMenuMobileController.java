...
public class RoleMenuMobileController {
    ...
    @GetMapping("/list")
    @ApiOperation(value = "获取菜单名称列表(物业)", notes = "获取菜单名称列表(物业)")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "Authorization", value = "授权token", required = true, paramType = "header"),
            @ApiImplicitParam(name = "TENANT-ID", value = "租户Id", required = true, paramType = "header"),
            @ApiImplicitParam(name = "PROJECT-ID", value = "小区Id", required = true, paramType = "header")
    })
    public R<List<String>> listPermissionByRoleId(@RequestHeader("PROJECT-ID") Integer projectId){
        ...
        if (ObjectUtil.isNotEmpty(userDeptRole.getData())){
            for (Object datum : userDeptRole.getData()) {
                ...
                if (jsonObject.get("deptId",Integer.class).equals(projectId)) {
                    return remoteRoleMenuMobileService.listPermissionByRoleId(jsonObject.get("roleId",Integer.class));
                }
            }
        }
       return R.ok();
    ...
    @GetMapping("/tree")
    @ApiOperation(value = "获取菜单结构树(物业)", notes = "获取菜单结构树(物业)")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "Authorization", value = "授权token", required = true, paramType = "header"),
            @ApiImplicitParam(name = "TENANT-ID", value = "租户Id", required = true, paramType = "header"),
            @ApiImplicitParam(name = "PROJECT-ID", value = "小区Id", required = true, paramType = "header")
    })
    public R<List<SysMenuMobileTreeVo>> treeRoleId(@RequestHeader("PROJECT-ID") Integer projectId) {
        R<List<Object>> userDeptRole = remoteUmsUserService.getUserDeptRole();
        if (ObjectUtil.isNotEmpty(userDeptRole.getData())){
            for (Object datum : userDeptRole.getData()) {
                JSONObject jsonObject = JSONUtil.parseObj(datum);
    ...
    public R<List<SysMenuMobileTreeVo>> gettreeRoleId(@PathVariable Integer roleId) {
