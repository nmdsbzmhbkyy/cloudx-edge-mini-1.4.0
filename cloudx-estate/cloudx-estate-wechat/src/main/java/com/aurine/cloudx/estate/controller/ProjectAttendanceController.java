...
public class ProjectAttendanceController {
    ...
    public R<Page<ProjectAttendanceCountPageVo>> page (Page page, @RequestParam("date") String date, @RequestParam(value = "deptId", required = false) Integer deptId) {
        ProjectStaffVo staffVo = projectStaffService.getStaffByOwner();
    ...
    public R<ProjectDeptCountAttendanceVo> sumCount (@RequestParam("date") String date, @RequestParam(value = "deptId", required = false) Integer deptId) {
        ProjectStaffVo staffVo = projectStaffService.getStaffByOwner();
        if (PositionEnum.STAFF.code.equals(staffVo.getGrade())) {
            return R.failed("员工不能查看");
        }
