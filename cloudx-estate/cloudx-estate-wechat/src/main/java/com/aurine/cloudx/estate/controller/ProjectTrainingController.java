...
@Api(value = "/projectTraining", tags = "员工培训")
...
//    private RemoteProjectTrainingService remoteProjectTrainingService;
    ...
    @ApiOperation(value = "获取员工全部培训列表", notes = "获取员工全部培训列表")
    @ApiImplicitParams({
    ...
    public R<List<TrainingStaffDetailVo>> listTrainByStaff(TrainingStaffDetailVo trainingStaffDetailVo) {
        List<TrainingStaffDetailVo> trainingStaffDetailVos = projectTrainingService.listTrainByStaff(trainingStaffDetailVo);
        LocalDateTime time = LocalDateTime.now();
        List<TrainingStaffDetailVo> list = trainingStaffDetailVos.stream()
            .map(e -> {
                String status = "1";
                if (time.isBefore(e.getBeginTime())) {
                    status = "0";
                }
                if (time.isAfter(e.getEndTime())) {
                    status = "2";
                }
                e.setStatus(status);
                return e;
            })
            .filter(e -> StringUtil.isEmpty(trainingStaffDetailVo.getStatus()) || e.getStatus().equals(trainingStaffDetailVo.getStatus()))
     ...
     */
    @GetMapping("/page")
    @ApiOperation(value = "获取员工培训列表", notes = "获取员工培训列表")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "Authorization", value = "授权token", required = true, paramType = "header"),
            @ApiImplicitParam(name = "TENANT-ID", value = "租户Id", required = true, paramType = "header"),
            @ApiImplicitParam(name = "PROJECT-ID", value = "小区Id", required = true, paramType = "header"),
    ...
    @ApiOperation(value = "根据培训ID获取培训资料", notes = "根据培训ID获取培训资料")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "Authorization", value = "授权token", required = true, paramType = "header"),
            @ApiImplicitParam(name = "TENANT-ID", value = "租户Id", required = true, paramType = "header"),
            @ApiImplicitParam(name = "PROJECT-ID", value = "小区Id", required = true, paramType = "header"),
...
//        return remoteProjectTrainingService.getTraining(trainingId);
    ...
    @ApiOperation(value = "根据文件id列表获取文件信息", notes = "根据文件id列表获取文件信息")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "Authorization", value = "授权token", required = true, paramType = "header"),
            @ApiImplicitParam(name = "TENANT-ID", value = "租户Id", required = true, paramType = "header"),
            @ApiImplicitParam(name = "PROJECT-ID", value = "小区Id", required = true, paramType = "header"),
    ...
    @ApiOperation(value = "把资料从未读状态改为已读状态", notes = "把资料从未读状态改为已读状态")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "Authorization", value = "授权token", required = true, paramType = "header"),
            @ApiImplicitParam(name = "TENANT-ID", value = "租户Id", required = true, paramType = "header"),
            @ApiImplicitParam(name = "PROJECT-ID", value = "小区Id", required = true, paramType = "header")
...
//        return remoteProjectTrainingService.setProgress(projectTrainingStaffDetail);
    ...
    @ApiOperation(value = "获取员工已读资料数", notes = "获取员工已读资料数")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "Authorization", value = "授权token", required = true, paramType = "header"),
            @ApiImplicitParam(name = "TENANT-ID", value = "租户Id", required = true, paramType = "header"),
            @ApiImplicitParam(name = "PROJECT-ID", value = "小区Id", required = true, paramType = "header"),
