...
public class ProjectDeviceRegionServiceImpl extends ServiceImpl<ProjectDeviceRegionMapper, ProjectDeviceRegion> implements ProjectDeviceRegionService {
    ...
    // 设备类型
    private final static String CAMERA_TYPE_AI = "19";
    // 区域ID
    ...
    public List<ProjectMonitorNodeVo> findMonitorTreeByDeviceType(String name) {
        ...
        List<ProjectDeviceInfo> list2 = projectDeviceInfoService.list(Wrappers.lambdaQuery(ProjectDeviceInfo.class)
                .eq(ProjectDeviceInfo::getDeviceType, CAMERA_TYPE_AI));
        list.addAll(list2);
...
//            }
        ...
        for (ProjectDeviceInfo info : list) {
            ProjectMonitorNodeVo node = new ProjectMonitorNodeVo();
            List<ProjectDeviceInfoVo> childDev = projectDeviceInfoService.getChildDev(info.getDeviceId(), null, null);
            if(CollectionUtil.isNotEmpty(childDev)) {
                for (ProjectDeviceInfoVo projectDeviceInfo :
                        childDev) {
                    ProjectMonitorNodeVo node2 = new ProjectMonitorNodeVo();
                    node2.setId(projectDeviceInfo.getDeviceId());
                    node2.setParentId(info.getDeviceId());
                    node2.setName(projectDeviceInfo.getDeviceName());
                    node2.setType(ProjectMonitorNodeVo.TYPE_MONITOR);
                    nodes.add(node2);
                }
            }
            if(info.getDeviceType().equals(CAMERA_TYPE_AI)) {
                node.setType(ProjectMonitorNodeVo.TYPE_REGION);
            } else {
                node.setType(ProjectMonitorNodeVo.TYPE_MONITOR);
            }
            node.setId(info.getDeviceId());
            node.setParentId(info.getDeviceRegionId() == null ? "1" : info.getDeviceRegionId());
...
//        return projectDeviceRegionMapper.initInsert(vo, projectId, tenantId);
    ...
    @Override
    public Page<ProjectBuildingRegionInfoVo> pageBuildingRegionInfo(Page<ProjectBuildingRegionInfoVo> page, String buildingName, String regionId,String regionName) {
        if (projectEntityLevelCfgService.checkIsEnabled()) {
            return projectDeviceRegionMapper.pageBuildingRegionInfoGroup(page, buildingName , regionId, regionName);
        }
        return projectDeviceRegionMapper.pageBuildingRegionInfo(page, buildingName , regionId, regionName);
    }
    ...
    public boolean moveBuildingsRegion(List<String> buildingIdList, String regionId) {
        ...
        List<String> deviceIdList = projectDeviceInfoProxyService.list(Wrappers.lambdaQuery(ProjectDeviceInfo.class)
                .in(ProjectDeviceInfo::getBuildingId, buildingIdList))
                .stream().map(ProjectDeviceInfo::getDeviceId).collect(Collectors.toList());
    ...
    @Override
    public List<ProjectBuildingInfoVo> listBuildingByRegionId(String regionId) {
    ...
    @Override
    public String getRegionIdByName(String key, String regionName) {
        if (StrUtil.isEmpty(key) || StrUtil.isEmpty(regionName)) {
            log.warn("设备导入区域ID获取缺少必要参数 redis键值:{} regionName:{}", key, regionName);
            return null;
        }
        if (!redisTemplateAurine.hasKey(key)) {
            synchronized (this) {
                if (!redisTemplateAurine.hasKey(key)) {
                    List<DeviceRegionInfoVo> regionInfoList = baseMapper.listAllRegionInfo();
                    Map<String, String> regionMap = regionInfoList.stream().collect(Collectors.toMap(DeviceRegionInfoVo::getRegionName, DeviceRegionInfoVo::getRegionId, (s, s2) -> s2));
                    regionMap.put("1","1");
                    redisTemplateAurine.opsForHash().putAll(key, regionMap);
                    redisTemplateAurine.expire(key, 1, TimeUnit.HOURS);
                }
            }
        }
        return (String) redisTemplateAurine.opsForHash().get(key, regionName);
    }

    @Override
    public ProjectDeviceRegion getPublicRegion() {
        return this.getOne(new QueryWrapper<ProjectDeviceRegion>().lambda()
