 ...
 */
@Service
public class SyncPersonPlanRelServiceImpl implements SyncService {
    ...
    public void perform(Integer projectId, String projectUUID) {
        ...
        int count = projectPersonPlanRelService.count();
        int current = (int) Math.ceil(((double) count / syncConfig.getNumber()));
        for (int i = 1; i <= current; i++) {
            Page<ProjectPersonPlanRel> page = projectPersonPlanRelService.page(new Page<>(i, syncConfig.getNumber()));
            page.getRecords().forEach(e -> {
                OpenApiEntity openApiEntity = SyncUtil.getOpenApiEntityByOperate(projectUUID, OpenPushSubscribeCallbackTypeEnum.OPERATE.name, OpenApiOperateTypeEnum.ADD.name, OpenApiServiceNameEnum.PERSON_PLAN_REL.name, e);
    ...
    public String getVersion() {
        return TableNameEnum.PERSON_PLAN_REL.code;
    }
}
