 ...
 */
@Service
public class SyncPersonLiftRelServiceImpl implements SyncService {
    ...
    public void perform(Integer projectId, String projectUUID) {
        ...
        LambdaQueryWrapper<ProjectPersonLiftRel> wrapper = Wrappers.lambdaQuery(ProjectPersonLiftRel.class).eq(ProjectPersonLiftRel::getProjectId, projectId);

        int count = projectPersonLiftRelService.count(wrapper);
        int current = (int) Math.ceil(((double) count / syncConfig.getNumber()));
        for (int i = 1; i <= current; i++) {
            Page<ProjectPersonLiftRel> page = projectPersonLiftRelService.page(new Page<>(i, syncConfig.getNumber()));
            page.getRecords().forEach(e -> {
                OpenApiEntity openApiEntity = SyncUtil.getOpenApiEntityByOperate(projectUUID, OpenPushSubscribeCallbackTypeEnum.OPERATE.name, OpenApiOperateTypeEnum.ADD.name, OpenApiServiceNameEnum.PERSON_LIFT_REL.name, e);
    ...
    public String getVersion() {
        return TableNameEnum.PERSON_LIFT_REL.code;
    }
}
