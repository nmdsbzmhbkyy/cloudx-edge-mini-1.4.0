...
public class ProjectPatrolRouteConfServiceImpl extends ServiceImpl<ProjectPatrolRouteConfMapper, ProjectPatrolRouteConf> implements ProjectPatrolRouteConfService {
     ...
     * @param vo
     */
    private void check(ProjectPatrolRouteConfVo vo) {
        //判断巡更点的数量是否为0
        ...
        //班次验证
        if (StringUtils.equals(vo.getPatrolTimeType(), "1")) {//每日相同
            if (CollUtil.isEmpty(vo.getRegularTimeList())) {
                throw new RuntimeException("请添加班次");
            }else{
                this.checkListIsEmpty(vo.getRegularTimeList());
            }
        } else {//按周配置
            if (CollUtil.isEmpty(vo.getMon()) && CollUtil.isEmpty(vo.getTue()) && CollUtil.isEmpty(vo.getWed()) && CollUtil.isEmpty(vo.getThu()) && CollUtil.isEmpty(vo.getFri()) && CollUtil.isEmpty(vo.getSat()) && CollUtil.isEmpty(vo.getSun())) {
                throw new RuntimeException("请c'd添加班次");
            }else{
                this.checkListIsEmpty(vo.getMon());
                this.checkListIsEmpty(vo.getTue());
                this.checkListIsEmpty(vo.getWed());
                this.checkListIsEmpty(vo.getThu());
                this.checkListIsEmpty(vo.getFri());
                this.checkListIsEmpty(vo.getSat());
                this.checkListIsEmpty(vo.getSun());
            }
        }
    }
     ...
     * @return
     */
    private void checkListIsEmpty(List<String> list) {
        if(CollUtil.isNotEmpty(list)){
            for (String str : list) {
                if (StringUtils.isEmpty(str)) {
                    throw new RuntimeException("请填写班次时间");
                }
            }
        }
    }
    ...
    public ProjectPatrolRouteConfVo getVoById(String patrolRouteId) {
        ...
        if ("1".equals(vo.getPatrolTimeType())) {
        ...
        for (ProjectPatrolPointConf projectPatrolPointConf : pointPoList) {
            if ("0".equals(projectPatrolPointConf.getStatus())) {
                continue;
            }
