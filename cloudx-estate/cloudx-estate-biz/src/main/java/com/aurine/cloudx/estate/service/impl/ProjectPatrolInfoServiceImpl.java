...
public class ProjectPatrolInfoServiceImpl extends ServiceImpl<ProjectPatrolInfoMapper, ProjectPatrolInfo> implements ProjectPatrolInfoService {
    ...
    @Resource
    ProjectWebSocketService projectWebSocketService;
...
//    }
    ...
    public boolean savePatrolInfo() {
        ...
        boolean flag = this.saveBatch(patrolInfoList);
    ...
    public boolean deleteById(String patrolId) {
        ...
        通过ID删除巡更记录
         */
        boolean flag = this.removeById(patrolId);
        WebSocketNotifyUtil.sendMessgae(ProjectContextHolder.getProjectId().toString(), JSONObject.toJSONString(projectWebSocketService.findNumByProjectId()));
...
//            patrolInfo.setPlanName(plan.getPlanName());
            ...
            if (CollUtil.isNotEmpty(staffIdList)) {
                ...
                if ("2".equals(route.getAssignType())) {//系统轮询
                    ...
                    if (CollUtil.isNotEmpty(personIdList)) {
                        ...
                        projectPatrolPersonService.assignPerson(patrolInfo.getPatrolId(), staffList, detailList, "auto");
                    }
                }
            }
