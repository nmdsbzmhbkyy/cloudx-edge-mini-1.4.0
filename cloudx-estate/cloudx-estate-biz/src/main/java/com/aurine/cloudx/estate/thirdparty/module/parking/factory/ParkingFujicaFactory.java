 ...
 * @date: 2021-03-15 15:28
 * @Copyright:
 */
@Component
public class ParkingFujicaFactory extends AbstractParkingFactory implements ApplicationContextAware {

    private static ParkingFujicaFactory factoryInstance;//工厂单例
    ...
    public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        //注入工厂实现
        ParkingFujicaFactory.factoryInstance = applicationContext.getBean(ParkingFujicaFactory.class);

        //注入各版本实现
        Map<String, ParkingService> map = applicationContext.getBeansOfType(ParkingService.class);
        map.forEach((key, instance) -> {
            if (instance.getPlatform().equals(ParkingAPICompanyEnum.FUJICA.name())) {
