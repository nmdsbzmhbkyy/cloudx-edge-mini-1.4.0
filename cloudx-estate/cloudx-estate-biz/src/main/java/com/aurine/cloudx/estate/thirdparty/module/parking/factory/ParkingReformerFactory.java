 ...
 * @version 1.0
 * @author： 林功鑫
 * @date： 2021-06-09 09:18
 */
@Component
public class ParkingReformerFactory extends AbstractParkingFactory implements ApplicationContextAware {

    private static ParkingReformerFactory factoryInstance;//工厂单例
    ...
    public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        //注入工厂实现
        ParkingReformerFactory.factoryInstance = applicationContext.getBean(ParkingReformerFactory.class);
        //注入各版本实现
        Map<String, ParkingService> map = applicationContext.getBeansOfType(ParkingService.class);
        map.forEach((key, instance) -> {
            if (instance.getPlatform().equals(ParkingAPICompanyEnum.REFORMER.name())) {
    ...
    public ParkingService getParkingService() {
        if (parkingServiceInstanceMap.get(versionEnum.code) == null){
            throw new RuntimeException("未找到Reformer ParkingService实例:" + versionEnum.code);
        }
