...
@Component
@Slf4j
public class AurineDataConnector {
    ...
    private final int MAX_LOOP = 3;//最大递归层数
...
//        RedisUtil.set(requestObject.getMessageId(), JSONObject.toJSONString(requestObject), 30);
        ...
        if (useMq) {
            ...
            return sendData(requestObject, requestObject.getMethod().equalsIgnoreCase(AurineMethodConstant.POST));
        }
    }
     ...
     * @return
     */
    public JSONObject sendData(AurineRequestObject requestObject, boolean isPost) {
     ...
     * @return
     */
    private JSONObject sendData(AurineRequestObject requestObject, boolean isPost, int loop) {
        if (loop >= MAX_LOOP) {
            log.error("冠林中台 {}", "凭证获取失败次数过多");
        ...
        try {
            ...
            while (StringUtils.isEmpty(AurineTokenUtil.getToken(requestObject.getConfig().getProjectId(), requestObject.isProject())) && (count <= 5)) {
                log.error("冠林中台 {}： 尝试第{}次登录", requestObject.getProjectId(), count);
                login(requestObject);
                count++;
            }
            ...
            if (e.getRawStatusCode() == HttpStatus.UNAUTHORIZED.value()) {
                ...
                AurineTokenUtil.removeToken(requestObject.getConfig().getProjectId(), requestObject.isProject());
                /**
...
//                    this.sendToMq(requestObject);
     ...
     */
//    @Async
    public void login(AurineRequestObject requestObject) throws ResourceAccessException {
...
//                AurineKafkaUtil.start();
                log.error("冠林中台 {}： {}", requestObject.getProjectId(), "连接异常");
            }

        } else {
            log.info("冠林中台{}： {}", requestObject.getProjectId(), "其他线程正在登录");
        }
    }
...
//        //检查登录锁，不存在则上锁，存在忽略该操作
//        log.info("冠林中台 项目ID:{} {}", requestObject.getProjectId(), "开始执行登录业务");
//        if (!AurineTokenUtil.hasLogin(requestObject.getProjectId())) {
//            //1 登录锁
//            AurineTokenUtil.loginLock(requestObject.getProjectId());
//            log.info("冠林中台： {} {}", requestObject.getProjectId(), "挂登陆锁");
//
//            //2 暂停kafka消费
//            AurineKafkaUtil.stop();
...
//                    //4 登录成功，重设Token
//                    log.info("获取到Token:{}", tokenResult.getJSONObject("body").getString("accessToken"));
//                    AurineTokenUtil.setToken(tokenResult.getJSONObject("body").getString("accessToken"), requestObject.getProjectId());
//                }
//
