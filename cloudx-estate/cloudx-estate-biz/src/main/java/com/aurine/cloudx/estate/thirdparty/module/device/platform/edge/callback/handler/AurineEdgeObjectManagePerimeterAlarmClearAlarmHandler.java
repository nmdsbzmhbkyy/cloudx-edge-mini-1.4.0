...
@Component
@Slf4j
public class AurineEdgeObjectManagePerimeterAlarmClearAlarmHandler extends AbstractHandler<CallBackData> {
    @Resource
    ...
    public boolean filter(CallBackData handleEntity) {
        if (!StringUtils.equals(AurineEdgeServiceEnum.CLEAR_ALARM.code, handleEntity.getOnNotifyData().getObjManagerData().getServiceId())) {
            return false;
        }
        ...
        log.info("[冠林边缘网关] {} 开始处理周界报警消除报警结果", handleEntity);
        return true;
    }
    ...
    public boolean doHandle(CallBackData handleEntity) {
        try {
            ...
            //获取下发的设备
            if (deviceInfo == null) {
                log.error("[冠林边缘网关] 周界报警消除报警失败，未找到下发的设备：deviceThirdCode = {}", deviceThirdCode);
                return done();
            }
...
//
            ...
            log.error("[冠林边缘网关] 周界报警消除报警结果处理失败，数据结构错误 {}", handleEntity);

        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            log.info("[冠林边缘网关] 周界报警消除报警结果处理结束 {}", handleEntity.getOnNotifyData().getObjManagerData().getObjInfo());
            return done();
        }
    }
}
