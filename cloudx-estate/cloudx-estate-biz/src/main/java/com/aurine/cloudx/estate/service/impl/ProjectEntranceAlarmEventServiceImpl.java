...
//        }
        ...
        for (ProjectEntranceAlarmEvent po : poList) {
            //存入创建人id
            PigxUser user = SecurityUtils.getUser();
            String userName = projectStaffService.getOne(new LambdaQueryWrapper<ProjectStaff>()
                    .eq(ProjectStaff::getUserId, user.getId())
                    .eq(ProjectStaff::getProjectId, ProjectContextHolder.getProjectId())).getStaffName();
            po.setPersonName(userName);
    ...
    public boolean save(ProjectEntranceAlarmEventVo vo) {
        ...
        /**********************开放平台openv2的数据实时推送*********************/
        KafkaProducer.sendMessage(TopicConstant.ALARM_EVENT,po);
        /*********************************************************************/
