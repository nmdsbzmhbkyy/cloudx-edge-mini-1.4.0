 ...
 */
@Service
public class SyncBuildingInfoServiceImpl implements SyncService {
    ...
    public void perform(Integer projectId, String projectUUID) {
        ...
        int count = projectBuildingInfoService.count();
        int current = (int) Math.ceil(((double) count / syncConfig.getNumber()));
        for (int i = 1; i <= current; i++) {
            Page<ProjectBuildingInfo> page = projectBuildingInfoService.page(new Page<>(i, syncConfig.getNumber()));
            page.getRecords().forEach(e -> {
                OpenApiEntity openApiEntity = SyncUtil.getOpenApiEntityByOperate(projectUUID, OpenPushSubscribeCallbackTypeEnum.OPERATE.name, OpenApiOperateTypeEnum.ADD.name, OpenApiServiceNameEnum.BUILDING_INFO.name, e);
    ...
    public String getVersion() {
        return TableNameEnum.BUILDING_INFO.code;
    }
}
