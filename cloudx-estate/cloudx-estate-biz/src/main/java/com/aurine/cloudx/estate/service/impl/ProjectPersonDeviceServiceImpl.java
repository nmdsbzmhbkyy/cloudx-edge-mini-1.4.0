...
public class ProjectPersonDeviceServiceImpl extends ServiceImpl<ProjectPersonDeviceMapper, ProjectPersonDevice> implements ProjectPersonDeviceService {
    ...
    public boolean savePersonDevice(ProjectPersonDeviceDTO personDeviceDTO) {
        ...
        List<ProjectPassDeviceVo> liftByPlanList = new ArrayList<>();//电梯（虚拟）设备集合


        boolean isExist = false;
        //删除旧数据
        this.deleteByPersonId(personDeviceDTO.getPersonId());
        this.projectPersonLiftRelService.deleteByPersonId(personDeviceDTO.getPersonId());
        ...
        liftByPlanList  = this.projectPersonLiftRelService.listLiftByPersonIdAndPlanId(personDeviceDTO.getPersonId(), planId);
        /**
         * 只保存用户手动选择的数据
         */
        //转换用户选择的设备列表
        ...
        if (projectPersonDeviceList.size() > 0) {
            for (ProjectPersonDevice personDevice : projectPersonDeviceList) {
                this.save(personDevice);
            }
        ...
        List<String> liftIdList = personDeviceDTO.getLiftIdList();
        if(CollectionUtil.isNotEmpty(liftIdList)){
            for (String id : liftIdList) {

                //如果设备已经存在于方案列表中，不在写入系统
                isExist = liftByPlanList.stream().anyMatch(e -> e.getDeviceId().equals(id));
                if (isExist) {
                    continue;
                }

                projectPersonLiftRel = new ProjectPersonLiftRel();
                BeanUtils.copyProperties(personDeviceDTO, projectPersonLiftRel);
                projectPersonLiftRel.setDeviceId(id);
                projectPersonLiftRel.setStatus("1");
                projectPersonLiftRel.setEffTime(personDeviceDTO.getEffTime());
                projectPersonLiftRel.setExpTime(personDeviceDTO.getExpTime());
                projectPersonLiftRelList.add(projectPersonLiftRel);
            }
        }
        if(CollectionUtil.isNotEmpty(projectPersonLiftRelList)){
            for (ProjectPersonLiftRel personLift:
            projectPersonLiftRelList) {
                this.projectPersonLiftRelService.save(personLift);
            }
        }
...
//        }
        ...
        //排除重复的电梯设备
        if(CollectionUtil.isNotEmpty(liftByPlanList)){
            if(liftIdList == null){
                liftIdList = new ArrayList<>();
            }
            for (ProjectPassDeviceVo liftVo:
            liftByPlanList) {
                if(!liftIdList.contains(liftVo.getDeviceId())){
                    liftIdList.add(liftVo.getDeviceId());
                }
            }
        }
        List<String> liftLadderList = projectDeviceInfoService.getLiftLadderList(personDeviceDTO.getPersonId());
        if(CollUtil.isNotEmpty(liftLadderList)){
            liftIdList.addAll(liftLadderList);
        }
        ...
        projectRightDeviceService.saveRelationship(new ArrayList<>(deviceIdList), personDeviceDTO.getPersonId());
...
//                    projectPersonDeviceList.add(projectPersonDevice);F
...
//        }
        ...
        // 更新人脸黑名单信息
        if (device.getDeviceType().equals(DeviceTypeConstants.GATE_DEVICE) || device.getDeviceType().equals(DeviceTypeConstants.LADDER_WAY_DEVICE)) {
            if (StringUtils.isNotBlank(device.getDeviceCapabilities()) && device.getDeviceCapabilities().contains("face")) {
                projectRightDeviceService.saveFaceBlacklistByDeviceId(device.getDeviceId());
            }
        }
    ...
    public boolean disablePassRight(String personType, String personId) {
        ...
        //人员电梯关系表
        List<ProjectPersonLiftRel> personLiftRelList = this.projectPersonLiftRelService.listByPersonId(personId);


        for (ProjectPersonDevice personDevice : list) {
            personDevice.setIsActive(PassRightActiveTypeEnum.DISABLE.code);
        }
        for (ProjectPersonLiftRel personLiftRel : personLiftRelList) {
            personLiftRel.setIsActive(PassRightActiveTypeEnum.DISABLE.code);
        }
        ...
        if (CollUtil.isNotEmpty(personLiftRelList)) {

            this.projectPersonLiftRelService.updateBatchById(personLiftRelList);
        }
    ...
    public boolean refreshByPersonId(String personId, PersonTypeEnum personTypeEnum) {
        ...
        if (personTypeEnum == PersonTypeEnum.PROPRIETOR) {
            ...
            if (houseCount <= 0) {
                log.info("[刷新住户权限] 住户无房屋：{}", personId);
        ...
        log.info("[刷新住户权限] 根据方案重载住户权限：{}", JSON.toJSONString(dto));
...
//        }
    ...
    /**
     * 根据人员ID 和 方案ID 获得该用户可以使用的设备VO列表(根据规则，仅方案内的设备)
     *
     * @param personId
     * @param planId
     * @return
     */
    @Override
    public List<ProjectPassDeviceVo> listDeviceByPersonIdAndPlanIdOnlyPlan(String personId, String planId) {
        List<String> macroList = projectPassPlanPolicyRelService.listMacroIdListByPlanId(planId);
    ...
    @Override
    public List<String> findPerson(List<String> planId, String buildingId, String unitId, String groupId){
        return this.baseMapper.findPerson(planId,buildingId,unitId,groupId);
    }
