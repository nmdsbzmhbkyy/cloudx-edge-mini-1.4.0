 ...
 */
@Service
public class SyncPassPlanPolicyRelServiceImpl implements SyncService {
    ...
    public void perform(Integer projectId, String projectUUID) {
        ...
        int count = projectPassPlanPolicyRelService.count();
        int current = (int) Math.ceil(((double) count / syncConfig.getNumber()));
        for (int i = 1; i <= current; i++) {
            Page<ProjectPassPlanPolicyRel> page = projectPassPlanPolicyRelService.page(new Page<>(i, syncConfig.getNumber()));
            page.getRecords().forEach(e -> {
                OpenApiEntity openApiEntity = SyncUtil.getOpenApiEntityByOperate(projectUUID, OpenPushSubscribeCallbackTypeEnum.OPERATE.name, OpenApiOperateTypeEnum.ADD.name, OpenApiServiceNameEnum.PASS_PLAN_POLICY_REL.name, e);
    ...
    public String getVersion() {
        return TableNameEnum.PASS_PLAN_POLICY_REL.code;
    }
