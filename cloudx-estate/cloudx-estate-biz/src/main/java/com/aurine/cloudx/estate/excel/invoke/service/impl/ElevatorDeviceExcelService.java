...
import com.aurine.cloudx.estate.excel.entity.access.ElevatorDeviceExcel;
 ...
 */
@Service
public class ElevatorDeviceExcelService extends BaseExcelRowInvokeService<ElevatorDeviceExcel> {
    ...
    @Override
    public RowInvokeResult excelRowInvoke(String batchId, ElevatorDeviceExcel data, AnalysisContext context, boolean enablePolice) {
        ...
        //电梯分层控制器ip地址必填
        if(StrUtil.equals(DeviceTypeConstants.ELEVATOR_LAYER_CONTROL_DEVICE,deviceInfo.getDeviceType())){
            if(StrUtil.isBlank(deviceInfo.getIpv4())){
                return RowInvokeResult.failed("ipv4未填写");
            }
        }

        //乘梯识别终端设备编号必填
        if(StrUtil.equals(DeviceTypeConstants.ELEVATOR_RECOGNIZER_DEVICE,deviceInfo.getDeviceType())){
            if(StrUtil.isBlank(deviceInfo.getDeviceCode())){
                return RowInvokeResult.failed("设备编号未填写");
            }
        }
    ...
    @Override
    public RowInvokeResult excelRowInvoke(String batchId, ElevatorDeviceExcel data, AnalysisContext context, boolean enablePolice, boolean isCover) {
        return null;
    }
    ...
    public Set<String> getDeviceTypeSet() {
        ...
        deviceTypeSet.add(DeviceTypeConstants.ELEVATOR_LAYER_CONTROL_DEVICE);
        deviceTypeSet.add(DeviceTypeConstants.ELEVATOR_RECOGNIZER_DEVICE);
