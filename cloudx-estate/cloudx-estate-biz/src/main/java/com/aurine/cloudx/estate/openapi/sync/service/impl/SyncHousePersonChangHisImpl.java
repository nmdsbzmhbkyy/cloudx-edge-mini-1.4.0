 ...
 *
 * @author：zouyu
 * @data: 2022-4-8 09:21:16
 */
@Service
public class SyncHousePersonChangHisImpl implements SyncService {
    ...
    public void perform(Integer projectId, String projectUUID) {
        ...
        int count = projectHousePersonChangeHisService.count();
        int current = (int) Math.ceil(((double) count / syncConfig.getNumber()));
        for (int i = 1; i <= current; i++) {
            Page<ProjectHousePersonChangeHis> page = projectHousePersonChangeHisService.page(new Page<>(i, syncConfig.getNumber()));
            page.getRecords().forEach(e -> {
                OpenApiEntity openApiEntity = SyncUtil.getOpenApiEntityByOperate(projectUUID, OpenPushSubscribeCallbackTypeEnum.OPERATE.name, OpenApiOperateTypeEnum.ADD.name, OpenApiServiceNameEnum.HOUSE_PERSON_CHANGE_HIS.name, e);;
    ...
    public String getVersion() {
        return TableNameEnum.HOUSE_PERSON_CHANGE_HIS.code;
    }
}
