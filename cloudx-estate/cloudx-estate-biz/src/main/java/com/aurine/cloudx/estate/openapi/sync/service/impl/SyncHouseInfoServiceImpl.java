 ...
 */
@Service
public class SyncHouseInfoServiceImpl implements SyncService {
    ...
    public void perform(Integer projectId, String projectUUID) {
        ...
        int count = projectHouseInfoService.count();
        int current = (int) Math.ceil(((double) count / syncConfig.getNumber()));
        for (int i = 1; i <= current; i++) {
            Page<ProjectHouseInfo> page = projectHouseInfoService.page(new Page<>(i, syncConfig.getNumber()),Wrappers.lambdaQuery(ProjectHouseInfo.class).orderByAsc(ProjectHouseInfo::getSeq));
            page.getRecords().forEach(e -> {
                OpenApiEntity openApiEntity = SyncUtil.getOpenApiEntityByOperate(projectUUID, OpenPushSubscribeCallbackTypeEnum.OPERATE.name, OpenApiOperateTypeEnum.ADD.name, OpenApiServiceNameEnum.HOUSE_INFO.name, e);
    ...
    public String getVersion() {
        return TableNameEnum.HOUSE_INFO.code;
    }
}
