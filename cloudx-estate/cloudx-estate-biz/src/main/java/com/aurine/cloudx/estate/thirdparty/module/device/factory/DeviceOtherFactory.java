 ...
 */
@Component
public class DeviceOtherFactory extends AbstractDeviceFactory implements ApplicationContextAware {

    private static DeviceOtherFactory factoryInstance;//工厂单例
    ...
    public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        //注入工厂实现
        DeviceOtherFactory.factoryInstance = applicationContext.getBean(DeviceOtherFactory.class);
    ...
    public PassWayDeviceService getPassWayDeviceService() {
        if (passWayDeviceServiceInstanceMap.get(versionEnum.code) == null){
            throw new RuntimeException("未找到Other PassWayDeviceService 实例:" + versionEnum.code);
        }
    ...
    public DeviceService getDeviceService() {
        if (deviceServiceInstanceMap.get(versionEnum.code) == null){
            throw new RuntimeException("未找到Other DeviceService 实例:" + versionEnum.code);
        }
    ...
    private <T extends BaseService> void initBean(Class<T> t, Map instanceMap, ApplicationContext applicationContext) throws BeansException {
        ...
        beanMap.forEach((key, instance) -> {
            if (instance.getPlatform().equals(PlatformEnum.OTHER.code)) {
