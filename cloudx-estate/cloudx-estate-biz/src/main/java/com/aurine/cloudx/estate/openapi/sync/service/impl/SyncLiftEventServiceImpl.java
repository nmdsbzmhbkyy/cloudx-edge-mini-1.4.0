 ...
 *
 * @author：zouyu
 * @data: 2022-07-18 10:38:58
 */
@Service
public class SyncLiftEventServiceImpl implements SyncService {
    ...
    public void perform(Integer projectId, String projectUUID) {
        ...
        LambdaQueryWrapper<ProjectLiftEvent> wrapper = Wrappers.lambdaQuery(ProjectLiftEvent.class).eq(ProjectLiftEvent::getProjectId, projectId);

        int count = projectLiftEventService.count(wrapper);
        int current = (int) Math.ceil(((double) count / syncConfig.getNumber()));
        for (int i = 1; i <= current; i++) {
            Page<ProjectLiftEvent> page = projectLiftEventService.page(new Page<>(i, syncConfig.getNumber()));
            page.getRecords().forEach(e -> {
                OpenApiEntity openApiEntity = SyncUtil.getOpenApiEntityByOperate(projectUUID, OpenPushSubscribeCallbackTypeEnum.OPERATE.name, OpenApiOperateTypeEnum.ADD.name, OpenApiServiceNameEnum.LIFT_EVENT.name, e);
    ...
    public String getVersion() {
        return TableNameEnum.LIFT_EVENT.code;
    }
}
