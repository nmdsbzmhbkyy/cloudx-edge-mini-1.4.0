...
import com.aurine.cloudx.estate.mapper.ProjectUnitInfoMapper;
...
public class ProjectUnitInfoServiceImpl extends ServiceImpl<ProjectUnitInfoMapper, ProjectUnitInfo> implements ProjectUnitInfoService {
    ...
    public boolean saveUnit(ProjectUnitInfo unitInfo, String buildingId,String unitNameType) {
        //init
...
//        unitInfo.setUnitId(uid);
        ...
        String letter = StringUtils.replace(unitInfo.getUnitName(), "单元", "");

        if(unitNameType.equals(BuildingUnitTypeEnum.TENANT.code)){
            ...
            String num = StringUtil.letterToNum(letter);

            letter = StringUtil.fillWithZero(codeRule, Integer.parseInt(num));
        }
        ...
        if (CollectionUtil.isNotEmpty(unitList)) {
            ...
            //保存单元编码和框架编码
            unitCode = projectFrameInfo.getEntityCode() + letter;
            unitInfo.setFrameNo(letter);
            //设置单元框架信息
            frameinfo.setEntityCode(unitCode);
            frameinfo.setFrameNo(letter);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public boolean saveBatchUnit(List<ProjectUnitInfo> unitInfoList, String buildingId,String unitNameType) {
        for (ProjectUnitInfo unitInfo : unitInfoList) {
            this.saveUnit(unitInfo, buildingId,unitNameType);
        }
        return true;
    }
