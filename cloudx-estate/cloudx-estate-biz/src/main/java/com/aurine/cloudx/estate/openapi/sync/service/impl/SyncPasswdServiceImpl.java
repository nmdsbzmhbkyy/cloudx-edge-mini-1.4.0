 ...
 */
@Service
public class SyncPasswdServiceImpl implements SyncService {
    ...
    public void perform(Integer projectId, String projectUUID) {
        ...
        int count = projectPasswdService.count();
        int current = (int) Math.ceil(((double) count / syncConfig.getNumber()));
        for (int i = 1; i <= current; i++) {
            Page<ProjectPasswd> page = projectPasswdService.page(new Page<>(i, syncConfig.getNumber()));
            page.getRecords().forEach(e -> {
                OpenApiEntity openApiEntity = SyncUtil.getOpenApiEntityByOperate(projectUUID, OpenPushSubscribeCallbackTypeEnum.OPERATE.name, OpenApiOperateTypeEnum.ADD.name, OpenApiServiceNameEnum.PASSWORD_INFO.name, e);
    ...
    public String getVersion() {
        return TableNameEnum.PASSWORD_INFO.code;
    }
}
