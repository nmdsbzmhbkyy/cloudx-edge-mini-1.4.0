...
public class ProjectStaffServiceImpl extends ServiceImpl<ProjectStaffMapper, ProjectStaff> implements ProjectStaffService {
    ...
    public boolean removeStaff(String id) {
        ...
        if (staff.getUserId() != null) {
            userRemote.removeUserRole(staff.getUserId(), staff.getRoleId());
        }
...
//        }
    ...
    public ProjectStaffVo getStaffByOwner() {
        /**
         * 通过调用SecurityContextHolder.getContext().getAuthentication()方法来获取当前登录的用户。
         * SecurityContextHolder是一个全局对象，用于存储当前登录的用户信息。
         * getContext()方法返回一个SecurityContext对象，其中包含了当前登录的用户信息。
         * getAuthentication()方法返回一个Authentication对象，其中包含了当前登录用户的身份信息。
         * */
        // 检查当前登录的用户是否为空，如果是，则返回null
    ...
    public List<SysUserVo> getUserVosByDeptId(Integer deptId) {
        ...
        return this.baseMapper.getUserVosByDeptId(deptId, "项目管理员");
    }
...
//        user.setRoleExpTime(staff.getRoleExpTime());
    ...
    @Override
    public boolean createAnAccount(ProjectStaffDTO entity) {
        CxUserDTO user = parseUser(entity);
        if (entity.getUserId() == null) {
            user.setPassword(PersonConstant.PASSWORD);
            user.setDeptId(entity.getDepartmentId());
            user.setUsername(entity.getMobile());
            user.setPhone(entity.getMobile());
            user.setNewRoleId(entity.getRoleId());
            R<Integer> r = userRemote.createAccount(user);
            if (r.getCode() ==  CommonConstants.SUCCESS) {
                entity.setUserId(r.getData());
    ...
    public String getDeptName(Integer departmentId) {
        return baseMapper.getDeptName(departmentId);
    }

    @Override
    public List<String> getProjectAdministrator(Integer projectId) {
        return baseMapper.getProjectAdministrator(projectId);
    }
}
