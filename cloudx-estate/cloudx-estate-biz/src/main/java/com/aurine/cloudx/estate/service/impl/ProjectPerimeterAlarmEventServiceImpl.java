...
import com.aurine.cloudx.estate.mapper.ProjectPerimeterAlarmEventMapper;
...
public class ProjectPerimeterAlarmEventServiceImpl extends ServiceImpl<ProjectPerimeterAlarmEventMapper, ProjectPerimeterAlarmEvent> implements ProjectPerimeterAlarmEventService {
    ...
    public Boolean deleteByEventId(String eventId) {
        ...
        boolean bool = DeviceFactoryProducer.getFactory(perimeterAlarmArea.getDeviceId()).getPerimeterAlarmDeviceService().clearAlarm(projectPerimeterAlarmEvent);
        if (!bool) {
            return bool;
        }
    ...
    @Override
    public Boolean saveAlarm(String deviceId, String module, String channel, String type) {
        ProjectPerimeterAlarmEvent projectPerimeterAlarmEvent = baseMapper.getAlarmArea(deviceId,module, channel);
        if (projectPerimeterAlarmEvent != null) {
            String id = UUID.randomUUID().toString().replace("-", "");
            projectPerimeterAlarmEvent.setEventId(id);
            projectPerimeterAlarmEvent.setAlaramTime(new Date());
    ...
    @Override
    public void updateAlarmEvent(String devId, String module, String channel, String errorType,String channelId) {
        ProjectPerimeterAlarmEvent projectPerimeterAlarmEvent = baseMapper.getAlarmEvent(devId,module,channel,errorType);
        if(projectPerimeterAlarmEvent != null){
            update(Wrappers.lambdaUpdate(ProjectPerimeterAlarmEvent.class)
                    .set(ProjectPerimeterAlarmEvent::getExecStatus, "1")
                    .eq(ProjectPerimeterAlarmEvent::getEventId, projectPerimeterAlarmEvent.getEventId()));
        }else{
            log.info("查询的报警事件为空 设备第三方ID:{},防区ID:{}",devId,channelId);
        }
