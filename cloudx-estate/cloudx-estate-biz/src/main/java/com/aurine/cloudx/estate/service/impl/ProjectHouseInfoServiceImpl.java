...
//            houseIdList.forEach(this::removeHouseAndFrameById);
    ...
    @Override
    public List<Integer> getFloorByBuildingId(String buildingId) {
        ProjectBuildingInfoVo buildingInfoVo = projectBuildingInfoService.getById(buildingId);
        if (StrUtil.isNotEmpty(buildingInfoVo.getFloorNumber())) {
            String[] strArr = buildingInfoVo.getFloorNumber().split(",");
            return Arrays.stream(strArr).mapToInt(Integer::parseInt).boxed().sorted(Comparator.naturalOrder()).collect(Collectors.toList());
        } else {
            return baseMapper.getFloorByBuildingId(buildingId);
        }
    }

    @Override
    public List<Integer> getFloorByProjectId(Integer projectId) {
        return baseMapper.getFloorByProjectId(projectId);
    }

    @Override
    public List<ProjectHouseInfoVo> getByBuildingId(String buildingId) {
        return baseMapper.getByBuildingId(buildingId);
    }

    @Override
    public List<String> getAddress(String houseId) {
        return baseMapper.getAddress(houseId);
    }

    @Override
    public List<String> getHouseParam(String personId) {
        return baseMapper.getHosueParam(personId);
    }

    @Override
    public R<String> getHouseId(Integer projectId) {
        ProjectContextHolder.setProjectId(projectId);
        String phone = SecurityUtils.getUser().getPhone();
        List<String> houseId= baseMapper.getHouseId(phone);
        if (CollUtil.isNotEmpty(houseId)){
            return R.ok(houseId.get(0));
        }else {
            return R.failed();
        }
    }
    ...
    @Override
    public List<ProjectHouseInfoVo> getHouseInfo() {
        Integer projectId = ProjectContextHolder.getProjectId();
        Integer id = SecurityUtils.getUser().getId();
        return baseMapper.getHouseInfo(id, projectId).stream().filter(e -> e != null).collect(Collectors.toList());
    }

    @Override
    public ProjectHouseInfo getFirstHouse(String personId, String buildingId, String unitId, String houseId) {
        ProjectHouseInfo house = null;
        //获取首套房信息
        List<ProjectHouseInfo> houselist = baseMapper.getHouseInfoByPersonAndBuild(personId, buildingId, unitId, houseId);
        if (CollUtil.isNotEmpty(houselist)) {
            house = houselist.get(0);
            house.setHouseName(house.getHouseName() + "房");
            //如果存在单位，用单位名称装饰房屋名称
            ProjectEmployerInfoFromVo employerInfoFromVo = projectEmployerInfoService.getByHouseId(house.getHouseId());

            if (employerInfoFromVo != null && houseId != null && StrUtil.isNotEmpty(employerInfoFromVo.getEmployerName())) {
                house.setHouseName(employerInfoFromVo.getEmployerName() + "(" + house.getHouseName()  + ")");
            }
        }
        return house;
    }
