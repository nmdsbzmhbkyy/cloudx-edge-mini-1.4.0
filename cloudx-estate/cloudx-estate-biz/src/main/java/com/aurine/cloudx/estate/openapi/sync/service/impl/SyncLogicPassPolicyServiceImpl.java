...
import com.aurine.cloudx.estate.service.ProjectLogicPassPolicyService;
 ...
 */
@Service
public class SyncLogicPassPolicyServiceImpl implements SyncService {
    ...
    public void perform(Integer projectId, String projectUUID) {
        ...
        int count = projectLogicPassPolicyService.count();
        int current = (int) Math.ceil(((double) count / syncConfig.getNumber()));
        for (int i = 1; i <= current; i++) {
            Page<ProjectLogicPassPolicy> page = projectLogicPassPolicyService.page(new Page<>(i, syncConfig.getNumber()));
            page.getRecords().forEach(e -> {
                OpenApiEntity openApiEntity = SyncUtil.getOpenApiEntityByOperate(projectUUID, OpenPushSubscribeCallbackTypeEnum.OPERATE.name, OpenApiOperateTypeEnum.ADD.name, OpenApiServiceNameEnum.LOGIC_PASS_POLICY.name, e);
    ...
    public String getVersion() {
        return TableNameEnum.LOGIC_PASS_POLICY.code;
    }
}
