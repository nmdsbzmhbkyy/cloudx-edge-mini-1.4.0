...
public class ProjectParkingPlaceServiceImpl extends ServiceImpl<ProjectParkingPlaceMapper, ProjectParkingPlace> implements ProjectParkingPlaceService {
    ...
    public IPage<ProjectParkingPlaceVo> findPage(IPage<ProjectParkingPlace> page, ProjectParkingPlaceVo projectParkingPlace) {
        return projectParkingPlaceMapper.select(page, ProjectContextHolder.getProjectId(), projectParkingPlace.getPlaceName(), projectParkingPlace.getParkRegionName(),
...
//        return complete;
    ...
    public boolean checkPlaceNameExist(List<String> placeNameList, String parkRegionId) {
        int num = this.count(new QueryWrapper<ProjectParkingPlace>().lambda()
                .in(ProjectParkingPlace::getPlaceName, placeNameList)
                .eq(ProjectParkingPlace::getParkRegionId, parkRegionId));
        if (num != 0) {
            return true;
        }
...
//        }
    ...
    public boolean savePlaceBatch(ProjectParkingPlaceVo parkingPlaceVo) {
        ...
        List<String> placeNameS = this.list(new QueryWrapper<ProjectParkingPlace>().lambda()
                        .in(ProjectParkingPlace::getPlaceName, parkingPlaceVo.getPlaceNameList())
                        .eq(ProjectParkingPlace::getParkRegionId, parkingPlaceVo.getParkRegionId())).stream()
                .map(projectParkingPlace -> projectParkingPlace.getPlaceName()).collect(Collectors.toList());
        List<ProjectParkingPlace> placeList = new ArrayList<>();
        parkingPlaceVo.getPlaceNameList().stream().map(placeName -> {
            if (ObjectUtil.isNotEmpty(placeNameS) && placeNameS.contains(placeName)) {
                return "";
            }
...
//        place.setRelType(PlaceRelTypeEnum.PUBLIC.code);//public按照规则，同时也是空置的定义
    ...
    @Override
    public boolean cancelParkingPlace(List<String> placeIdList) {
        boolean publicUpdateResult = false;
        boolean otherUpdateResult = false;
        if (CollUtil.isNotEmpty(placeIdList)) {
            // 更新公共车位
            publicUpdateResult = this.update(new UpdateWrapper<ProjectParkingPlace>().lambda()
                    .set(ProjectParkingPlace::getPersonId, "").set(ProjectParkingPlace::getPersonName, "")
                    .set(ProjectParkingPlace::getPlaceCode, "").set(ProjectParkingPlace::getPersonCode, "")
                    .set(ProjectParkingPlace::getRuleId, "").in(ProjectParkingPlace::getPlaceId, placeIdList)
                    .eq(ProjectParkingPlace::getRelType, PlaceRelTypeEnum.PUBLIC.code));
            // 更新非公共车位
            otherUpdateResult = this.update(new UpdateWrapper<ProjectParkingPlace>().lambda()
                    .set(ProjectParkingPlace::getRuleId, "")
                    .set(ProjectParkingPlace::getPersonCode, "")
                    .in(ProjectParkingPlace::getPlaceId, placeIdList)
                    .ne(ProjectParkingPlace::getRelType, PlaceRelTypeEnum.PUBLIC.code));
        }
        return publicUpdateResult || otherUpdateResult;
    }
    ...
    public List<ProjectParkingPlace> listParkPlaceByRelType(String parkRegionId, String personId, String relType, String placeId) {
        if (projectConfigService.isEnableMultiCarsPerPlace(ProjectContextHolder.getProjectId())) {
            return this.list(new QueryWrapper<ProjectParkingPlace>().lambda()
                    .eq(ProjectParkingPlace::getParkRegionId, parkRegionId)
                    .eq(ProjectParkingPlace::getRelType, relType)
                    .eq(ProjectParkingPlace::getPersonId, personId).or()
                    .eq(ProjectParkingPlace::getPlaceId, placeId));
        } else {
            return this.baseMapper.listParkPlaceByRelType(parkRegionId, personId, relType, placeId);
        }
    }
     ...
     */
    @Override
    public int countPrivatePlace() {
        return projectParkingPlaceMapper.countPrivatePlace();
    }

    @Override
    public Integer getUseTotal() {
        return projectParkingPlaceMapper.getUseTotal();
    }
    ...
    @Override
    public List<ProjectParkingPlace> getFreeParkingSpace(String personId) {
        return this.baseMapper.getFreeParkingSpace(personId);
    }
