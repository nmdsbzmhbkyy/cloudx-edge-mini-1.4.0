...
import com.aurine.cloudx.estate.dto.ProjectEntranceEventDto;
...
public class ProjectEntranceEventServiceImpl extends ServiceImpl<ProjectEntranceEventMapper, ProjectEntranceEvent> implements ProjectEntranceEventService {
    ...
    public boolean addEvent(ProjectEventVo vo) {
        ...
        boolean save = this.save(projectEntranceEvent);
        /**********************开放平台openv2的数据实时推送*********************/

        ProjectEntranceEventDto dto = new ProjectEntranceEventDto();
        BeanUtils.copyProperties(projectEntranceEvent, dto);
        dto.setThirdCertMediaId(vo.getThirdCertMediaId());
        dto.setQrcode(vo.getQrcode());
        KafkaProducer.sendMessage(TopicConstant.PEOPLE_PASS_RECORD,dto);
        /*********************************************************************/
        return save;
    }
