 ...
 */
@Service
public class SyncHouseDesignServiceImpl implements SyncService {
    ...
    public void perform(Integer projectId, String projectUUID) {
        ...
        LambdaQueryWrapper<ProjectHouseDesign> wrapper = Wrappers.lambdaQuery(ProjectHouseDesign.class).eq(ProjectHouseDesign::getProjectId, projectId);
        int count = projectHouseDesignService.count(wrapper);
        int current = (int) Math.ceil(((double) count / syncConfig.getNumber()));
        for (int i = 1; i <= current; i++) {
            Page<ProjectHouseDesign> page = projectHouseDesignService.page(new Page<>(i, syncConfig.getNumber()), wrapper);
            page.getRecords().forEach(e -> {
                OpenApiEntity openApiEntity = SyncUtil.getOpenApiEntityByOperate(projectUUID, OpenPushSubscribeCallbackTypeEnum.OPERATE.name, OpenApiOperateTypeEnum.ADD.name, OpenApiServiceNameEnum.HOUSE_DESIGN.name, e);
    ...
    public String getVersion() {
        return TableNameEnum.HOUSE_DESIGN.code;
    }
}
