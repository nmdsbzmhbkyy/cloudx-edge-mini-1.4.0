 ...
 */
@Component
public class IntercomDongDongFactory extends AbstractIntercomFactory implements ApplicationContextAware {

    private static IntercomDongDongFactory factoryInstance;//工厂单例
    ...
    public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        //注入工厂实现
        IntercomDongDongFactory.factoryInstance = applicationContext.getBean(IntercomDongDongFactory.class);
    ...
    public IntercomService getIntercomService() {
        if (intercomServiceInstanceMap.get(versionEnum.code) == null){
            throw new RuntimeException("未找到咚咚 IntercomUserDeviceService 实例:" + versionEnum.code);
        }
    ...
    private <T extends BaseService> void initBean(Class<T> t, Map instanceMap, ApplicationContext applicationContext) throws BeansException {
        ...
        beanMap.forEach((key, instance) -> {
            if (instance.getPlatform().equals(PlatformEnum.INTERCOM_DONGDONG.code)) {
