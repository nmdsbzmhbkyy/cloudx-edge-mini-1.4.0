...
import com.aurine.cloudx.estate.constant.EdgeCascadeServiceIdConstant;
...
public class EdgeCascadeRequestMasterServiceImpl extends ServiceImpl<EdgeCascadeRequestMasterMapper, EdgeCascadeRequestMaster> implements EdgeCascadeRequestMasterService {
    ...
    @Resource
    EdgeCascadeRequestMasterAspectService edgeCascadeRequestMasterAspectService;
    @Resource
    ...
    public R<EdgeCascadeResponse> requestCascade(CascadeRequestInfoDTO requestInfo) {
        ...
        edgeCascadeRequestMasterAspectService.requestCascadeSaveOrUpdate(cascadeRequestMaster);
        /*if (StrUtil.isNotEmpty(cascadeRequestMaster.getRequestId())) {
            edgeCascadeRequestMasterAspectService.updatePo(cascadeRequestMaster);
        } else {
            edgeCascadeRequestMasterAspectService.requestCascadeSaveOrUpdatePo(cascadeRequestMaster);
        }*/
        ...
        edgeCascadeService.pubSubCfg(new SubCfgInfo("0", cascadeRequestMaster.getProjectCode()));
        ...
        edgeCascadeService.confDriverCommunityId(new DriverManagerReq(masterUUID));
    ...
    public R<String> passRequest(String requestId) {
        ...
        edgeCascadeRequestMasterAspectService.passRequestUpdate(requestMaster);
//        this.updateById(requestMaster);
    ...
    public R<String> passRequestBatch(List<String> requestIdList) {
        requestIdList.forEach(edgeCascadeRequestMasterService::passRequest);
        return R.ok("操作成功");
    }
    ...
    public R<String> rejectRequest(String requestId) {
        ...
        if (requestMaster != null) {
            ...
            this.edgeCascadeRequestMasterAspectService.rejectRequestUpdate(requestMaster);
            this.updateById(requestMaster);
    ...
    public Integer countSlave() {
        return this.count(new LambdaQueryWrapper<EdgeCascadeRequestMaster>()
                .in(EdgeCascadeRequestMaster::getCascadeStatus, CascadeStatusConstants.CASCADE, CascadeStatusConstants.PENDING_AUDIT));
    }
...
//                throw new RuntimeException("新增失败,该用户已有该角色");
    ...
    public R<String> removeEdge(Integer projectId) {
        ...
        edgeCascadeRequestMasterAspectService.removeEdgeUpdate(requestMaster);
//        this.updateById(requestMaster);
        return R.ok("删除成功");
    }
    ...
    public R<Page<CascadeProjectInfoVo>> pageCascadeManage(Page page, CascadeManageQuery cascadeManageQuery) {
        ...
        if (requestMaster != null) {
            ...
            // 默认第一个项目就是主边缘网关项目
            masterProjectId = this.getOriginProjectId();
        ...
        Integer masterProjectId = this.getOriginProjectId();
        return R.ok(baseMapper.pageCascadeManage(page, cascadeManageQuery, masterProjectId));
    }

    @Override
    public R<Page<EdgeCascadeRequestMaster>> listRequest(Page page, Integer projectId) {
        // 万一没有项目ID
        if (projectId != null) {
            return R.ok(this.page(page, new LambdaQueryWrapper<EdgeCascadeRequestMaster>().eq(EdgeCascadeRequestMaster::getCascadeStatus, CascadeStatusConstants.PENDING_AUDIT)));
        }
        return R.ok(this.page(page, new LambdaQueryWrapper<EdgeCascadeRequestMaster>().eq(EdgeCascadeRequestMaster::getProjectId, projectId).eq(EdgeCascadeRequestMaster::getCascadeStatus, CascadeStatusConstants.PENDING_AUDIT)));
    }

    @Override
    public R cancelRequest(String projectCode) {
        ...
        requestMaster.setCascadeStatus(CascadeStatusConstants.NOT_CASCADE);
        this.edgeCascadeRequestMasterAspectService.cancelRequestUpdate(requestMaster);
//        this.updateById(requestMaster);
        edgeCascadeResponse.setResultCode(EdgeCascadeEventCodeEnum.CANCEL_CASCADE_REQUEST_SUCCESS.code);
        return R.ok(edgeCascadeResponse);
    }
    ...
    @Override
    @Cacheable(value = "originProjectCache", key = "'originProject'")
