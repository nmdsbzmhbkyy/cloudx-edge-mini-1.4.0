...
import com.aurine.parking.entity.dto.ProjectParCarRegisterDto;
import com.aurine.parking.feign.RemoteExitLaneService;
import com.aurine.parking.feign.RemoteParkingBullingRuleService;
...
public class ProjectVisitorServiceImpl extends ServiceImpl<ProjectVisitorMapper, ProjectVisitor> implements ProjectVisitorService {
    ...
    @Resource
    private RemoteParkingCarRegisterService remoteParkingCarRegisterService;

    @Resource
    private RemoteParkingBullingRuleService remoteParkingBullingRuleService;
    ...
    public boolean register(ProjectVisitorVo projectVisitorVo) {
        ...
        // 保存人和电梯关系
        if(CollUtil.isNotEmpty(projectVisitorVo.getLifts())){
            projectPersonLiftRelService.savePersonDevice(visitId, PersonTypeEnum.VISITOR,
                    projectVisitorVo.getLifts().stream().map(ProjectDeviceLiftVo::getDeviceId).collect(Collectors.toList()),
                    effTime, expTime);
        }
...
//        List<ProjectPasswd> passwordList = projectVisitorVo.getPasswordList();
        ...
        projectVisitorHis.setPassBeginTime(timeRange[0]);
        projectVisitorHis.setPassEndTime(timeRange[1]);
            ...
            if (config.getVisitorAudit().equals(ProjectConfigConstant.SYSTEM_IDENTITY)) {
                ...
                if (CollUtil.isNotEmpty(staffIdList)) {
                    String message = MessageTextUtil.init()
                            ...
                            .p("来访日期：%s", DateTimeFormatter.ofPattern("yyyy年MM月dd日 HH时mm分").format(LocalDateTime.parse(projectVisitorHis.getPassBeginTime(), DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"))))
   ...
   /*    // 20230704 V1.3.0.0（人行版）去除车场相关业务
        //如果有登记车辆
        if(StringUtils.isNotEmpty(projectVisitorVo.getPlateNumber())){
            ProjectParCarRegisterDto projectParCarRegisterDto = new ProjectParCarRegisterDto();
            List<String> plateNumberList = new ArrayList<>();
            plateNumberList.add(projectVisitorVo.getPlateNumber());
            projectParCarRegisterDto.setPlateNumberArray(plateNumberList);
            projectParCarRegisterDto.setPlateNumber(projectVisitorVo.getPlateNumber());
            projectParCarRegisterDto.setParkId(projectVisitorVo.getParkId());
            projectParCarRegisterDto.setPersonName(projectVisitorVo.getPersonName());
            projectParCarRegisterDto.setTelephone(projectVisitorVo.getMobileNo());
            projectParCarRegisterDto.setRuleId(remoteParkingBullingRuleService.getFreeRule(projectParCarRegisterDto.getParkId()).getData());
            projectParCarRegisterDto.setRelType("0");
            projectParCarRegisterDto.setLaneList(projectVisitorVo.getLaneList());
            projectParCarRegisterDto.setVisitBeginTime(LocalDateTime.parse(projectVisitorHis.getPassBeginTime(),DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")));
            projectParCarRegisterDto.setVisitEndTime(LocalDateTime.parse(projectVisitorHis.getPassEndTime(),DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")));
            projectParCarRegisterDto.setStartTime(projectParCarRegisterDto.getVisitBeginTime().toLocalDate());
            projectParCarRegisterDto.setEndTime(projectParCarRegisterDto.getVisitEndTime().toLocalDate());
            remoteParkingCarRegisterService.register(projectParCarRegisterDto);
        }*/
        return save;
    }
        ...
        if (CollUtil.isNotEmpty(visitorHisList)) {
            ...
            TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronizationAdapter() {
                ...
                public void afterCommit() {
                    String passBeginTimeStr = projectVisitorHis.getPassBeginTime();
                    DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern(DatePattern.NORM_DATETIME_PATTERN);
                    LocalDateTime passBeginTime = LocalDateTime.parse(passBeginTimeStr, dateTimeFormatter);

                    // 这里获取到通行结束时间的LocalDateTime对象 (延时任务添加)
                    LocalDateTime passEndDateTime = LocalDateTime.parse(projectVisitorHis.getPassEndTime(), dateTimeFormatter);
...
//                DelayTaskUtil.instance().addDelayTask(new DelayTask(passBeginTime, ProjectContextHolder.getProjectId(), DelayTaskTopicEnum.visitorSendCert));
                ...
                if (auditStatus.equals(AuditStatusEnum.pass)) {
                    ...
                    if (CollUtil.isNotEmpty(staffIdList)) {
                        String message = MessageTextUtil.init()
                                ...
                                .p("来访日期：%s", DateTimeFormatter.ofPattern("yyyy年MM月dd日 HH时mm分").format(LocalDateTime.parse(visitorHis.getPassBeginTime(), DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))))
    ...
    public boolean signOff(String visitId) {
       ...
       /*    // 20230704 V1.3.0.0（人行版）去除车场相关信息
            //签离时 如果访客车辆，迁出车辆
            if(StringUtils.isNotEmpty(projectVisitorHis.getPlateNumber())){
                remoteParkingCarRegisterService.cancelCarRegister(remoteParkingCarRegisterService.getRegisterByPlateNumber(projectVisitorHis.getPlateNumber()).getData().getRegisterId());
            }*/
    ...
    public Boolean passAuditBatch(List<ProjectVisitorVo> visitorVoList) {
        if (CollUtil.isNotEmpty(visitorVoList)) {
            ...
            threadPoolTaskExecutor.execute(() -> {
                ...
                projectVisitorHis.stream().forEach(visitorHis -> {
...
//        String visitorId = visitorHis.getVisitorId();
        ...
        if (StrUtil.isNotBlank(visitId)) {
            ...
            List<ProjectPersonLiftRel> projectPersonLiftRelList =  projectPersonLiftRelService.listByPersonId(visitId);
            liftIdList.addAll(projectPersonLiftRelList.stream().map(ProjectPersonLiftRel::getDeviceId).collect(Collectors.toList()));
            //设备列表加入乘梯识别终端
...
//        removeAuthByVisitId(visitId);
    ...
    public void sendCert(ProjectVisitorHis hisData) {
        ...
        String timeFormat3 = "yyyy-MM-dd HH:mm:ss";
        String eff = hisData.getPassBeginTime();
        String exp = hisData.getPassEndTime();
        DateTimeFormatter df = DateTimeFormatter.ofPattern(timeFormat3);
        LocalDateTime effTime = LocalDateTime.parse(eff, df);
        LocalDateTime expTime = LocalDateTime.parse(exp, df);
        ...
        //梯区口设备
        List<String> deviceIdList = new ArrayList<>();
        List<ProjectPersonDevice> projectPersonDeviceList = projectPersonDeviceService.listByPersonId(hisData.getVisitId());
        deviceIdList.addAll(projectPersonDeviceList.stream().map(ProjectPersonDevice::getDeviceId).collect(Collectors.toList()));
        //电梯设备
        List<String> liftIdList = new ArrayList<>();
        List<ProjectPersonLiftRel> projectPersonLiftRelList =  projectPersonLiftRelService.listByPersonId(hisData.getVisitId());
        liftIdList.addAll(projectPersonLiftRelList.stream().map(ProjectPersonLiftRel::getDeviceId).collect(Collectors.toList()));
...
//        this.signOffAll();
    ...
    @Override
    public Page<ProjectVisitor> select(Page page) {
        return baseMapper.select(page);
    }
}
