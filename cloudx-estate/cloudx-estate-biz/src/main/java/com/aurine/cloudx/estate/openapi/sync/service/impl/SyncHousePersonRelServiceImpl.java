 ...
 */
@Service
public class SyncHousePersonRelServiceImpl implements SyncService {
    ...
    public void perform(Integer projectId, String projectUUID) {
        ...
        int count = projectHousePersonRelService.count();
        int current = (int) Math.ceil(((double) count / syncConfig.getNumber()));
        for (int i = 1; i <= current; i++) {
            Page<ProjectHousePersonRel> page = projectHousePersonRelService.page(new Page<>(i, syncConfig.getNumber()));
            page.getRecords().forEach(e -> {
                OpenApiEntity openApiEntity = SyncUtil.getOpenApiEntityByOperate(projectUUID, OpenPushSubscribeCallbackTypeEnum.OPERATE.name, OpenApiOperateTypeEnum.ADD.name, OpenApiServiceNameEnum.HOUSE_PERSON_INFO.name, e);;
    ...
    public String getVersion() {
        return TableNameEnum.HOUSE_PERSON_INFO.code;
    }
}
