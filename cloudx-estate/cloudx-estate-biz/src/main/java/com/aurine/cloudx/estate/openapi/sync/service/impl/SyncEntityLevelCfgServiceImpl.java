 ...
 */
@Service
public class SyncEntityLevelCfgServiceImpl implements SyncService {
    ...
    public void perform(Integer projectId, String projectUUID) {
        ...
        LambdaQueryWrapper<ProjectEntityLevelCfg> wrapper = Wrappers.lambdaQuery(ProjectEntityLevelCfg.class).eq(ProjectEntityLevelCfg::getProjectId, projectId);
        int count = projectEntityLevelCfgService.count(wrapper);
        int current = (int) Math.ceil(((double) count / syncConfig.getNumber()));
        for (int i = 1; i <= current; i++) {
            Page<ProjectEntityLevelCfg> page = projectEntityLevelCfgService.page(new Page<>(i, syncConfig.getNumber()), wrapper);
            page.getRecords().forEach(e -> {
                OpenApiEntity openApiEntity = SyncUtil.getOpenApiEntityByOperate(projectUUID, OpenPushSubscribeCallbackTypeEnum.OPERATE.name, OpenApiOperateTypeEnum.ADD.name, OpenApiServiceNameEnum.ENTITY_LEVEL_CFG.name, e);
    ...
    public String getVersion() {
        return TableNameEnum.ENTITY_LEVEL_CFG.code;
    }
}
