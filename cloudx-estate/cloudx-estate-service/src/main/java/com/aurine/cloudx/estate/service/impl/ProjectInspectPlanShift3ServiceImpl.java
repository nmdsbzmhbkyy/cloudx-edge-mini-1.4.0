    ...
    public Map<String, List<ProjectInspectPlanShift3Vo>> getShiftListMapByPlanId(String planId, String cronType) {
        ...
        if (CollUtil.isNotEmpty(shift3List)) {
            shift3List.forEach(shift3 -> {
                ...
                if (stringListMap.containsKey(keyByCornType)) {
                    stringListMap.get(keyByCornType).add(shift3Vo);
                } else {
                    List<ProjectInspectPlanShift3Vo> voList = new ArrayList<>();
                    voList.add(shift3Vo);
                    stringListMap.put(keyByCornType, voList);
                }
            });
        }
