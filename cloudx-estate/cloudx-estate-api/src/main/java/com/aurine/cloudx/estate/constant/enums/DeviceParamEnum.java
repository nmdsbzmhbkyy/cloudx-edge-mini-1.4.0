...
//    QRCODE_OBJ("qrcodeParam", "QrcodeObj"),
    ...
    // 边缘网关电梯分层控制器-设备参数
    EDGE_LIFT_DEVICE_PARAMS_OBJ("LiftDeviceParamsObj", "LiftDeviceParamsObj"),
    // 边缘网关电梯分层控制器-网络参数
    EDGE_LIFT_NETWORK_PARAMS_OBJ("LiftNetworkParamsObj", "LiftNetworkParamsObj"),
    // 边缘网关电梯分层控制器-硬件参数
    EDGE_LIFT_HARDWARE_PARAMS_OBJ("LiftHardwareParamsObj", "LiftHardwareParamsObj"),
    // 边缘网关电梯分层控制器-基本参数
    EDGE_LIFT_BASE_PARAMS_OBJ("LiftBaseParamsObj", "LiftBaseParamsObj"),
    // 边缘网关电梯分层控制器-基本参数
    EDGE_LIFT_PERIOD_MANAGE_PARAMS_OBJ("LiftPeriodManageParamsObj", "LiftPeriodManageParamsObj"),
    // 边缘网关电梯分层控制器-丽人时段参数
    EDGE_LIFT_BEAUTY_PERIOD_PARAMS_OBJ("LiftBeautyPeriodParamsObj", "LiftBeautyPeriodParamsObj"),
    // 边缘网关电梯分层控制器-楼层设置
    EDGE_LIFT_FLOOR_OBJ("LiftFloorObj", "LiftFloorObj"),
    // 边缘网关电梯分层控制器-系统参数
    EDGE_LIFT_SYS_OPERATE_OBJ("LiftSysOperateObj", "LiftSysOperateObj"),
...
//    EXT_PARAM_OBJ("extParam", "ExtParamObj"),
    ...
    * */
    ...
    // 车道一体机参数 语言播报参数
    PARKING_DEVICE_VOICE("parkingDeviceVoice", "parkingDeviceVoice"),

    // 车道一体机参数 显示屏内容参数（前端页面是显示屏内容和显示屏设置）
    PARKING_DEVICE_SCREEN("parkingDeviceScreen", "parkingDeviceScreen"),

    // 车道一体机参数 其他系统参数设置
    PARKING_OTHER_SETTING("otherSetting", "SFMDeviceParams"),
    ;
...
/*
    ...
    public static List<DeviceParamEnum> getEnums(List<String> serviceIdList) {
        DeviceParamEnum[] values = DeviceParamEnum.values();
        List<DeviceParamEnum> paramEnumList = new ArrayList<>();
        Set<DeviceParamEnum> serviceIdSet = Arrays.stream(values).filter(paramEnum -> paramEnum.getServiceId().equals(serviceIdList)).collect(Collectors.toSet());

        for (DeviceParamEnum value : values) {
            // 这里是防止传入的是参数类型ID但是要获取的也是参数类型ID，两种都能获取到参数类型ID
            if (serviceIdSet.contains(value.getServiceId())) {
                paramEnumList.add(value);
            }
        }
