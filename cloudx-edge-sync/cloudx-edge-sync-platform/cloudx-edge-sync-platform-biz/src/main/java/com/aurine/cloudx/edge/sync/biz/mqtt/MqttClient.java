...
public class MqttClient {
     ...
     * 添加 MQTT订阅消息列表 - 云端
     * 主题类型：
     * edgeName/从UUID/edge/cloudSlave/0000/VERSION/0/0/request/CMD
     * edgeName/从UUID/edge/cloudSlave/0000/VERSION/0/0/response/CMD
     */
    public void addSubscribeTopics(String... projectUUIDList) {
        ...
        for (int i = 0; i < projectUUIDList.length; i += 1) {
            ProjectRelation byProjectCode = projectRelationService.getByProjectCode(projectUUIDList[i]);
            //订阅的主题,不同端代码不一致
            subscribeTopics[i * 2] = String.format(Constants.SUBSCRIBE_TOPIC_PREFIX, byProjectCode.getSn(), projectUUIDList[i], Constants.topicVersion, MessageTypeEnum.REQUEST.name);
            subscribeTopics[i * 2 + 1] = String.format(Constants.SUBSCRIBE_TOPIC_PREFIX, byProjectCode.getSn(), projectUUIDList[i], Constants.topicVersion, MessageTypeEnum.RESPONSE.name);
        }
     ...
     * 移除 MQTT订阅 - 边缘侧主从端
     * 主题类型：
     * 从UUID/edge/cloudSlave/0000/VERSION/0/0/request/CMD
     * 从UUID/edge/cloudSlave/0000/VERSION/0/0/response/CMD
     */
    public void removeSubscribeTopics(String sn, String projectUUID) {
        String[] subscribeTopics = new String[2];
        subscribeTopics[0] = String.format(Constants.SUBSCRIBE_TOPIC_PREFIX, sn, projectUUID, Constants.topicVersion, MessageTypeEnum.REQUEST.name);
        subscribeTopics[1] = String.format(Constants.SUBSCRIBE_TOPIC_PREFIX, sn, projectUUID, Constants.topicVersion, MessageTypeEnum.RESPONSE.name);
        log.info("移除MQTT订阅：{}", subscribeTopics);
