...
import com.aurine.cloudx.open.api.inner.feign.RemoteMetaBlacklistService;
 ...
 *
 * @author zy
 * @Date 2022-08-15 14:22:28
 */
@Slf4j
@Component
public class BlackListStrategy implements BaseStrategy {

    @Resource
    private RemoteMetaBlacklistService remoteMetaBlacklistService;

    @Override
    public OpenRespVo doAdd(OpenApiModel<JSONObject> requestObj) {
        log.info("[车场入云] {} add 请求数据:{}", ServiceNameEnum.BLACKLIST.name, JSON.toJSONString(requestObj));
        requestObj.getData().put("seq", null);
        OpenApiModel openApiModel = new OpenApiModel();
        openApiModel.setData(JSONObject.toJavaObject(requestObj.getData(), ProjectBlacklist.class));
        openApiModel.setHeader(requestObj.getHeader());

        R r = remoteMetaBlacklistService.save(openApiModel);
        log.info("[车场入云] {} add 响应数据:{}", ServiceNameEnum.BLACKLIST.name, JSON.toJSONString(r));
        if (r.getCode() != 0) {
        ...
        String id = object.getString("blackId");
    ...
    public OpenRespVo doDelete(String appId, String projectUUID, Integer tenantId, String uuid) {
        log.info("[车场入云] {} delete 请求数据:{}", ServiceNameEnum.BLACKLIST.name, uuid);
        R r = remoteMetaBlacklistService.delete(appId, projectUUID, tenantId, uuid);
        log.info("[车场入云] {} delete 响应数据:{}", ServiceNameEnum.BLACKLIST.name, uuid);
        return new OpenRespVo(null, r);
    }

    @Override
    public void afterPropertiesSet() throws Exception {
        BaseStrategyFactory.registerStrategy(ServiceNameEnum.BLACKLIST.name, this);
    }
}
