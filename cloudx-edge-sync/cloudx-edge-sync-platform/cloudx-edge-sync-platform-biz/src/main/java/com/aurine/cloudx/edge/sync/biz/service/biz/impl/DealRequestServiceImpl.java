...
import com.aurine.cloudx.edge.sync.biz.service.impl.DispatchServiceImpl;
...
public class DealRequestServiceImpl implements DealRequestService {
    ...
    @Resource
    RemoteCascadeCloudApplyService remoteCascadeCloudApplyService;
    ...
    public void dealRequest(TaskInfoDto taskInfoDto) {
        ...
        try {
            ...
            Integer projectId = DispatchServiceImpl.projectMap.get(taskInfoDto.getProjectUUID());
            if(projectId == null){
                projectId = taskInfoService.getProjectIdByProjectUuid(taskInfoDto.getProjectUUID());
                DispatchServiceImpl.projectMap.put(taskInfoDto.getProjectUUID(),projectId);
            }
    ...
    private R dispatchCascadeRequest(TaskInfoDto taskInfoDto) {
        ...
        ProjectRelation byProjectUUID = projectRelationService.getByProjectRelation(taskInfoDto.getProjectUUID(), taskInfoDto.getProjectCode());
    ...
    private void dealRequestCascade(TaskInfoDto taskInfoDto) {
        if (OpenPushSubscribeCallbackTypeEnum.CASCADE.name.equals(taskInfoDto.getServiceType())) {
            ...
            if (ServiceNameEnum.CASCADE_APPLY.name.equals(taskInfoDto.getServiceName())
                ...
                ProjectRelation relation = projectRelationService.getByProjectRelation(taskInfoDto.getProjectUUID(), taskInfoDto.getProjectCode());
                log.info("dealRequestCascade 执行撤销入云申请操作：{}", taskInfoDto);
                ...
                log.info("dealRequestCascade 再次更新入云申请状态：{}", revoke);
            }
        }
    }
    ...
    private OpenRespVo dispatchCommonRequest(TaskInfoDto taskInfoDto) {
        ...
        log.debug("baseStrategy:{}", baseStrategy);
        try {
    ...
    private OpenRespVo dispatchOtherRequest(TaskInfoDto taskInfoDto) {
        log.debug("[{}] dispatchOtherRequest{}", taskInfoDto.getMsgId(), taskInfoDto);
        try {
            switch (OperateTypeEnum.getByName(taskInfoDto.getType())) {
                case ADD:
    ...
    private void deaOtherRequest(TaskInfoDto taskInfoDto) {
        log.debug("[{}] 处理其他操作{}", taskInfoDto.getMsgId(), taskInfoDto);
        // 分发请求
...
//        updateDatabaseInfo(taskInfoDto, resp);
    ...
    private Boolean changeProjectUUIDToOwner(TaskInfoDto taskInfoDto) {
        ...
        if (ObjectUtil.isEmpty(byProjectCode)) {
            log.info("[{}] MQTT REQUEST 找不到ProjectUuid对应关系", taskInfoDto.getMsgId());
            return false;
        }
