...
@Component
@Slf4j
public class ImageToBase64FaceInfoHandler extends AbstractHandler<TaskInfoDto> {
    /**
    ...
    public boolean filter(TaskInfoDto handleEntity) {
        ...
        if (!ServiceNameEnum.FACE_INFO.name.equals(handleEntity.getServiceName())) {
            return false;
        }

        log.info("[边缘网关同步服务] {} ImageToBase64人脸信息图片数据转换", handleEntity);
        return true;
    }
    ...
    public boolean doHandle(TaskInfoDto handleEntity) {
        ...
        String picUrl = JSONObject.toJavaObject(jsonObj, ProjectFaceResources.class).getPicUrl();
