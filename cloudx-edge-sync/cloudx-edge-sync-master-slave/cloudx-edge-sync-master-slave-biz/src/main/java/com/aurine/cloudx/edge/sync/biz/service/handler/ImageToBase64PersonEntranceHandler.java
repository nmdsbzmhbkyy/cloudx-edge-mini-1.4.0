...
@Component
@Slf4j
public class ImageToBase64PersonEntranceHandler extends AbstractHandler<TaskInfoDto> {
    /**
    ...
    public boolean filter(TaskInfoDto handleEntity) {
        ...
        if (!ServiceNameEnum.PERSON_ENTRANCE.name.equals(handleEntity.getServiceName())) {
            return false;
        }

        log.info("[边缘网关同步服务] {} ImageToBase64人行事件图片数据转换", handleEntity);
    ...
    public boolean doHandle(TaskInfoDto handleEntity) {
        ...
        String picUrl = JSONObject.toJavaObject(jsonObj, ProjectEntranceEvent.class).getPicUrl();
