...
//        new URL(url).
     ...
     * @Auther: 王伟
     */
    public void saveToMinio(String url, String bucketName, String objectName) throws IOException {
        log.info("开始转存图片：{}", url);
        ...
        try {
            minioTemplate.putObject(bucketName, objectName, intStream, size, MediaType.IMAGE_JPEG_VALUE);
        } catch (ProtocolException e) {
            log.error("图片转存本地失败");
            e.printStackTrace();
        } catch (Exception e) {
...
//        new URL(url).
     ...
     * @Auther: 王伟
     */
    public String base64ToMinio(String imgBase64, String fileName) throws IOException {
        if (imgBase64 == null || imgBase64 == "") {
            return null;
        }
        log.info("开始转存图片：{}", fileName);
        ...
        StringBuilder fileNameDest = new StringBuilder();
        for (int i = 4; i < split.length; i++) {
            if (i == 4) {
                fileNameDest.append(split[i]);
            } else {
                fileNameDest.append("/").append(split[i]);
            }
        }
        log.info("bucketName = {}", bucketName);
        log.info("fileNameDest = {}", fileNameDest);
        try {
            minioTemplate.putObject(bucketName, fileNameDest.toString(), intStream, date_blob.length, MediaType.IMAGE_JPEG_VALUE);
            return fileName;
        } catch (ProtocolException e) {
            log.error("图片转存本地失败");
            e.printStackTrace();
