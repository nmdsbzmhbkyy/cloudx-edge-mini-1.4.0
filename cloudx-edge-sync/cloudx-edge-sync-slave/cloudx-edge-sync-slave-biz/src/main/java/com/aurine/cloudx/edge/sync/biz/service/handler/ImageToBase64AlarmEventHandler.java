...
@Component
@Slf4j
public class ImageToBase64AlarmEventHandler extends AbstractHandler<TaskInfoDto> {
    /**
    ...
    public boolean filter(TaskInfoDto handleEntity) {
        ...
        if (!ServiceNameEnum.ALARM_EVENT.name.equals(handleEntity.getServiceName())) {
            return false;
        }

        log.info("[边缘网关同步服务] {} ImageToBase64报警事件图片数据转换", handleEntity);
    ...
    public boolean doHandle(TaskInfoDto handleEntity) {
        ...
        String picUrl = JSONObject.toJavaObject(jsonObj, ProjectEntranceAlarmEvent.class).getPicUrl();
