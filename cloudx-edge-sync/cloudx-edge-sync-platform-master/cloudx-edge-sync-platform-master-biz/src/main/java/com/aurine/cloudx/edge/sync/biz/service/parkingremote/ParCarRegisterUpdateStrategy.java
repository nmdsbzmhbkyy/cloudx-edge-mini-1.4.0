...
@Slf4j
@Component
public class ParCarRegisterUpdateStrategy implements CommandStrategy {
    ...
    public OpenRespVo doHandler(TaskInfoDto taskInfoDto) {
        //调用第三方服务
        try {
            log.info("[车场入云] {} 开始调用第三方的接口:{}", CommandTypeEnum.CAR_INFO_UPDATE.desc,taskInfoDto);
            JSONObject jsonObject = JSONObject.parseObject(taskInfoDto.getData());
            R r = remoteProjectParkingService.carInfoUpdate(jsonObject, SecurityConstants.FROM_IN);
    ...
    public void afterPropertiesSet() throws Exception {
        CommandStrategyFactory.registerStrategy(CommandTypeEnum.CAR_INFO_UPDATE.name, this);
    }
}
