...
@Component
@Slf4j
public class Base64ToImageAlarmHandlerHandler extends AbstractHandler<TaskInfoDto> {
    @Resource
    ...
    public boolean filter(TaskInfoDto handleEntity) {
        ...
        log.info("[边缘网关同步服务] Base64ToImage报警处理图片数据转换");
    ...
    public boolean doHandle(TaskInfoDto handleEntity) {
        ...
        List base64List = jsonObj.getObject(PublicConstants.PIC_BASE64_KEY, List.class);
        String picUrl = jsonObj.getString("picUrl");
        if(StrUtil.isNotEmpty(picUrl) && picUrl.contains(",")){
            String[] urlArray = picUrl.split(",");
            if (CollUtil.isNotEmpty(base64List)) {
                for (int i = 0; i < base64List.size(); i++) {
                    imgConvertUtil.base64ToMinio(base64List.get(i).toString(), urlArray[i]);
                }
