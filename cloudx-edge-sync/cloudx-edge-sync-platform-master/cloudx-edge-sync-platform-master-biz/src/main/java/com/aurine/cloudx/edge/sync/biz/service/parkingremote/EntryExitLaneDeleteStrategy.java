...
@Slf4j
@Component
public class EntryExitLaneDeleteStrategy implements CommandStrategy {
    ...
    public OpenRespVo doHandler(TaskInfoDto taskInfoDto) {
        //调用第三方服务
        try {
            log.info("[车场入云] {} 开始调用第三方的接口:{}",CommandTypeEnum.ENTRY_EXIT_LANE_DELETE.desc,taskInfoDto);
            R r = remoteProjectParkingService.entryExitLaneDelete(taskInfoDto.getData(), SecurityConstants.FROM_IN);
    ...
    public void afterPropertiesSet() throws Exception {
        CommandStrategyFactory.registerStrategy(CommandTypeEnum.ENTRY_EXIT_LANE_DELETE.name, this);
    }
}
