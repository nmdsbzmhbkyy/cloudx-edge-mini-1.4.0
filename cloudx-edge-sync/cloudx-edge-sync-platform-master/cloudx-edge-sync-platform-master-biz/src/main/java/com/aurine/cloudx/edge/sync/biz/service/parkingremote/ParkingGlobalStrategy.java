...
@Slf4j
@Component
public class ParkingGlobalStrategy implements CommandStrategy {
    ...
    public OpenRespVo doHandler(TaskInfoDto taskInfoDto) {
        //调用第三方服务
        try {
            log.info("[车场入云] {} 开始调用第三方的接口:{}",CommandTypeEnum.PARKING_GLOBAL.desc,taskInfoDto);
            JSONObject jsonObject = JSONObject.parseObject(taskInfoDto.getData());
            R r = remoteProjectParkingService.parkingGlobal(jsonObject, SecurityConstants.FROM_IN);
    ...
    public void afterPropertiesSet() throws Exception {
        CommandStrategyFactory.registerStrategy(CommandTypeEnum.PARKING_GLOBAL.name, this);
    }
}
